      ...
      <div class="pizza-card">
        ...
        <label>Crust:
          <select id="crust_${p.pizza_id}">
            ${data.crusts.map(c => `<option value="${c.crust_id}" data-extra="${c.extra_price}">${c.crust_name} (+$${c.extra_price})</option>`).join('')}
          </select>
        </label>
        <label>Toppings:</label>
        ${data.toppings.map(t => `
          <label><input type="checkbox" class="topping_${p.pizza_id}" value="${t.topping_id}" data-extra="${t.extra_price}"> ${t.topping_name} (+$${t.extra_price})</label>
        `).join('')}
...
function addToCart(pizza_id, name, base_price) {
  ...
  const toppingBoxes = document.querySelectorAll(`.topping_${pizza_id}:checked`);
  const toppings = [...toppingBoxes].map(t => parseInt(t.value));
  const topping_extra = [...toppingBoxes].reduce((s, t) => s + parseFloat(t.dataset.extra), 0);
  const item_price = (parseFloat(base_price) + crust_extra + topping_extra) * qty;
...
function renderCart() {
  ...
  list.innerHTML = cart.map((c, i) => `<li>${c.name} x${c.quantity} – $${c.item_price.toFixed(2)}</li>`).join('');
...
function placeOrder() {
  ...
  .then(data => {
    if (data.success) {
      ...
      document.getElementById('orderMsg').innerHTML = `<p class="error">${data.error}</p>`;
    }
  });
}
</script>
